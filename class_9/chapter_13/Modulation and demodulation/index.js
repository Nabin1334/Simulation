const canvas = document.getElementById('simCanvas');
        ...
        inputs.audioFreq.addEventListener('input', (e) => {
            labels.freq.innerText = e.target.value < 2 ? "Low" : e.target.value > 4 ? "High" : "Med";
        });

        // Helper: Draw a sine wave
        function drawWave(ctx, xStart, yCenter, width, amplitude, frequency, phase, color, label) {
            ctx.beginPath();
        ...
        function drawAMWave(ctx, xStart, yCenter, width, cAmp, cFreq, mAmp, mFreq, phase) {
            ctx.beginPath();
        ...
        function drawIcon(ctx, type, x, y) {
            ...
            if (type === 'mic') {
                ...
                // Cross bars
                ctx.moveTo(x - 5, y + 15); ctx.lineTo(x + 5, y + 15);
                ctx.moveTo(x - 10, y + 35); ctx.lineTo(x + 10, y + 35);
                ctx.stroke();
        ...
        function animate() {
            ...
            ctx.beginPath(); ctx.strokeStyle = "#555"; ctx.moveTo(60, 100); ctx.lineTo(100, 100); ctx.stroke();
            
            drawWave(ctx, 100, 100, 150, mAmp, mFreq, time, "#00ff00", "Message Signal (Audio)");
            ...
            ctx.beginPath(); 
            ctx.moveTo(250, 100); ctx.lineTo(270, 100); // Audio in
            ctx.moveTo(250, 215); ctx.lineTo(270, 215); // Carrier in
            ctx.stroke();

            // 5. Transmitting Antenna
            // Line from modulator to antenna
            ctx.beginPath(); ctx.moveTo(350, 150); ctx.lineTo(400, 150); ctx.stroke();
            ...
            ctx.beginPath(); ctx.moveTo(700, 150); ctx.lineTo(750, 150); ctx.stroke();
            drawBox(ctx, "DEMODULATOR", 750, 110, 90, 80);

            // 8. Output (Speaker)
            // The demodulator extracts the "Envelope" (The Message)
            ctx.beginPath(); ctx.moveTo(840, 150); ctx.lineTo(880, 150); ctx.stroke();
            
            // Draw the recovered wave (same as message)
            drawWave(ctx, 880, 150, 80, mAmp, mFreq, time, "#ffff00", "Recovered Audio");
            
            // Connection to speaker
            ctx.beginPath(); ctx.moveTo(960, 150); ctx.lineTo(970, 150); ctx.stroke();
